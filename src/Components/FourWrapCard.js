import React from 'react'
import './FourWrapCard.css'

export default function FourWrapCard() {
  return (
    <div className='four-wrap-card'>
       <div className='two-card-1st-part'>
           <div className='single-card-two'>
            <div className='single-card-two-inner-d'>
              <div><img src="/Images/robotWrapCard1.svg"/></div>
               
               <div className='single-card-two-content'>
                     <div className='single-card-two-content-part1'><img src="/Images/mashinelearning.svg" /></div>
                     <div className='single-card-two-content-inner-d1'>
                      <div className='single-card-two-content-inner-d2'>
                          <div className='single-card-two-content-part2'><h2>Machine Learning</h2></div>
                          <div className='single-card-two-content-part3'><p>In publishing and graphic design, Lorem ipsum is a placeholder. </p></div>
                          </div>
                          <div className='Progress-bar-heading-part'>
                    <div className='Progress-bar-heading'><p>Learn More</p></div>
                      <div className='Progress-bar-pic'><img src="/Images/mainSectionArrow.svg" alt="" /></div>
                  </div>
                     </div>
               </div>
             
             </div>
           </div>
           {/* part 2 */}

           <div className='single-card-two'>
            <div className='single-card-two-inner-d'>
              <div><img src="/Images/pic2Card.svg"/></div>
               
               <div className='single-card-two-content'>
                     <div className='single-card-two-content-part1'><img src="/Images/pic2CardIcon.svg" /></div>
                     <div className='single-card-two-content-inner-d1'>
                      <div className='single-card-two-content-inner-d2'>
                          <div className='single-card-two-content-part2'><h2>Machine Learning</h2></div>
                          <div className='single-card-two-content-part3'><p>In publishing and graphic design, Lorem ipsum is a placeholder. </p></div>
                          </div>
                          <div className='Progress-bar-heading-part'>
                    <div className='Progress-bar-heading'><p>Learn More</p></div>
                      <div className='Progress-bar-pic'><img src="/Images/mainSectionArrow.svg" alt="" /></div>
                  </div>
                     </div>
               </div>
             
             </div>
           </div>


       </div>

       {/* Section Two */}
        
                    {/* Part 3 */}

       <div className='two-card-2st-part'>
           <div className='single-card-two'>
            <div className='single-card-two-inner-d'>
              <div><img src="/Images/pic3Card.svg"/></div>
               
               <div className='single-card-two-content'>
                     <div className='single-card-two-content-part1'><img src="/Images/pic3CardIcon.svg" /></div>
                     <div className='single-card-two-content-inner-d1'>
                      <div className='single-card-two-content-inner-d2'>
                          <div className='single-card-two-content-part2'><h2>Machine Learning</h2></div>
                          <div className='single-card-two-content-part3'><p>In publishing and graphic design, Lorem ipsum is a placeholder. </p></div>
                          </div>
                          <div className='Progress-bar-heading-part'>
                    <div className='Progress-bar-heading'><p>Learn More</p></div>
                      <div className='Progress-bar-pic'><img src="/Images/mainSectionArrow.svg" alt="" /></div>
                  </div>
                     </div>
               </div>
             
             </div>
           </div>
           {/* part 4 */}

           <div className='single-card-two'>
            <div className='single-card-two-inner-d'>
              <div><img src="/Images/pic4Card.svg"/></div>
               
               <div className='single-card-two-content'>
                     <div className='single-card-two-content-part1'><img src="/Images/pic4CardIcon.svg" /></div>
                     <div className='single-card-two-content-inner-d1'>
                      <div className='single-card-two-content-inner-d2'>
                          <div className='single-card-two-content-part2'><h2>Machine Learning</h2></div>
                          <div className='single-card-two-content-part3'><p>In publishing and graphic design, Lorem ipsum is a placeholder. </p></div>
                          </div>
                          <div className='Progress-bar-heading-part'>
                    <div className='Progress-bar-heading'><p>Learn More</p></div>
                      <div className='Progress-bar-pic'><img src="/Images/mainSectionArrow.svg" alt="" /></div>
                  </div>
                     </div>
               </div>
             
             </div>
           </div>


       </div>
       

    </div>
  )
}
